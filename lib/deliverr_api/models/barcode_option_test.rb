  ...
  class BarcodeOptionTest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DeliverrAPI::BarcodeOptionTest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DeliverrAPI::BarcodeOptionTest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'barcode_value')
        self.barcode_value = attributes[:'barcode_value']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @barcode_value.to_s.length > 255
        invalid_properties.push('invalid value for "barcode_value", the character length must be smaller than or equal to 255.')
      end

      if @barcode_value.to_s.length < 6
        invalid_properties.push('invalid value for "barcode_value", the character length must be great than or equal to 6.')
      end
    ...
    def valid?
      ...
      barcode_type_validator = EnumAttributeValidator.new('String', ["PRODUCT", "MANUFACTURER", "AMAZON"])
      return false unless barcode_type_validator.valid?(@barcode_type)
      return false if @barcode_value.nil?
      return false if @barcode_value.to_s.length > 255
      return false if @barcode_value.to_s.length < 6
      true
    end
    ...
    def barcode_type=(barcode_type)
      validator = EnumAttributeValidator.new('String', ["PRODUCT", "MANUFACTURER", "AMAZON"])
    ...
    # @param [Object] barcode_value Value to be assigned
    def barcode_value=(barcode_value)
      if barcode_value.nil?
        fail ArgumentError, 'barcode_value cannot be nil'
      end

      if barcode_value.to_s.length > 255
        fail ArgumentError, 'invalid value for "barcode_value", the character length must be smaller than or equal to 255.'
      end

      if barcode_value.to_s.length < 6
        fail ArgumentError, 'invalid value for "barcode_value", the character length must be great than or equal to 6.'
      end
    ...
    def hash
      [barcode_type, barcode_value].hash
    end
