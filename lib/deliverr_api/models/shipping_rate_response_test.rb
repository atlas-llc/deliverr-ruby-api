...
module DeliverrAPI
  class ShippingRateResponseTest
    # The name of the shipping rate to display to the customer
    attr_accessor :name

    # An identifier for the shipping rate
    attr_accessor :code

    # The number of transit days for the shipping rate
    attr_accessor :shipping_days

    # A message with the arrival date
    attr_accessor :arrives_by

    attr_accessor :price

    # Additional information to share with the customer
    attr_accessor :note
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DeliverrAPI::ShippingRateResponseTest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DeliverrAPI::ShippingRateResponseTest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'shipping_days')
        self.shipping_days = attributes[:'shipping_days']
      end

      if attributes.key?(:'arrives_by')
        self.arrives_by = attributes[:'arrives_by']
      end
    ...
    def list_invalid_properties
      ...
      if @shipping_days.nil?
        invalid_properties.push('invalid value for "shipping_days", shipping_days cannot be nil.')
      end

      if @arrives_by.nil?
        invalid_properties.push('invalid value for "arrives_by", arrives_by cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @shipping_days.nil?
      return false if @arrives_by.nil?
    ...
    def hash
      [name, code, shipping_days, arrives_by, price, note].hash
    end
