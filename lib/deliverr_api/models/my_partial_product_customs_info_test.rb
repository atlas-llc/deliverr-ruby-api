...
module DeliverrAPI
  ...
  class MyPartialProductCustomsInfoTest
    # Customs description - should reflect usage and materials. Needed only for international shipping.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DeliverrAPI::MyPartialProductCustomsInfoTest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DeliverrAPI::MyPartialProductCustomsInfoTest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@customs_description.nil? && @customs_description.to_s.length < 1
    ...
    def valid?
      ...
      return false if !@customs_description.nil? && @customs_description.to_s.length < 1
      return false if !@harmonized_code.nil? && @harmonized_code !~ Regexp.new(/^[0-9]{10}$/)
      return false if !@customs_value.nil? && @customs_value < 0.01
      true
    end
