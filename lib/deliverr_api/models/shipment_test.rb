...
module DeliverrAPI
  class ShipmentTest
    # Deliverr's internal identifier for the shipment.
    ...
    # All items of this shipment.
    attr_accessor :items

    # All packages of this shipment.
    ...
    def self.openapi_types
      {
        :'shipment_id' => :'Float',
        :'status' => :'ApiInboundShipmentStatusTest',
        :'ship_to_address' => :'AddressTest',
        :'items' => :'Array<ShipmentItemTest>',
        :'packages' => :'Array<PackageTest>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DeliverrAPI::ShipmentTest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DeliverrAPI::ShipmentTest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [shipment_id, status, ship_to_address, items, packages].hash
    end
