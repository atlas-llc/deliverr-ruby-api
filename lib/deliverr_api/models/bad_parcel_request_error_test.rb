...
module DeliverrAPI
  class BadParcelRequestErrorTest
    attr_accessor :code

    attr_accessor :message

    # An optional error message for incorrectly formatted requests to indicate why the request failed. Example: \"child \\\"dimensions\\' fails because [child \\\"weightUnit\\' fails because [\\\"weightUnit\\\" must be one of [lb, kg]]]\" This indicates that the request failed because the weightUnit that was provided is not supported.
    attr_accessor :type
    ...
    def self.openapi_types
      {
        :'code' => :'StatusCodesBADREQUESTTest',
        :'message' => :'String',
        :'type' => :'String'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DeliverrAPI::BadParcelRequestErrorTest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DeliverrAPI::BadParcelRequestErrorTest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
