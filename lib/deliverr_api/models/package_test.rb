  ...
  class PackageTest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DeliverrAPI::PackageTest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DeliverrAPI::PackageTest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'cdsku')
        self.cdsku = attributes[:'cdsku']
      end
    ...
    def list_invalid_properties
      ...
      if @cdsku.nil?
        invalid_properties.push('invalid value for "cdsku", cdsku cannot be nil.')
      end
    ...
    def hash
      [length, width, height, weight, dimension_unit, weight_unit, cdsku, items].hash
    end
