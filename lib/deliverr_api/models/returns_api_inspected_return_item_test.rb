...
module DeliverrAPI
  class ReturnsApiInspectedReturnItemTest
    attr_accessor :product

    # Quantity of product send when creating return order.
    attr_accessor :expected_quantity

    # Inspection details once the product is scan you can get more or less than expectedQuantity in this payload.
    ...
    def self.openapi_types
      {
        :'product' => :'ReturnsApiProductDetailTest',
        :'expected_quantity' => :'Float',
        :'inspection_details' => :'Array<ReturnsApiInspectionDetailTest>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DeliverrAPI::ReturnsApiInspectedReturnItemTest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DeliverrAPI::ReturnsApiInspectedReturnItemTest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'expected_quantity')
        self.expected_quantity = attributes[:'expected_quantity']
      end

      if attributes.key?(:'inspection_details')
        if (value = attributes[:'inspection_details']).is_a?(Array)
          self.inspection_details = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @expected_quantity.nil?
        invalid_properties.push('invalid value for "expected_quantity", expected_quantity cannot be nil.')
      end

      if @inspection_details.nil?
        invalid_properties.push('invalid value for "inspection_details", inspection_details cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @expected_quantity.nil?
      return false if @inspection_details.nil?
      true
    end
    ...
    def hash
      [product, expected_quantity, inspection_details].hash
    end
