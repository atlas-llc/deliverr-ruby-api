...
module DeliverrAPI
  class QuoteCreateResponseTest
    ...
    # Warnings received while getting a quote.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DeliverrAPI::QuoteCreateResponseTest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DeliverrAPI::QuoteCreateResponseTest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @billed_weight_lbs.nil?
        invalid_properties.push('invalid value for "billed_weight_lbs", billed_weight_lbs cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @billed_weight_lbs.nil?
    ...
    def hash
      [price, service_level, billed_weight_lbs, dimensions, warnings].hash
    end
