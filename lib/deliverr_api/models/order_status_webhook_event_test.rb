...
module DeliverrAPI
  class OrderStatusWebhookEventTest
    # The UTC timestamp of the triggering event.
    attr_accessor :event_time

    # The globally unique id of this event - can be used for deduplication or idempotency purposes.
    ...
    #  The shipment details if the update_type is ORDER_SHIPPED. One shipment is reported per package shipped.
    ...
    def self.openapi_types
      {
        :'event_time' => :'Time',
        :'event_id' => :'String',
        :'type' => :'WebhookTypeORDERSTATUSUPDATETest',
        :'update_type' => :'WebhookOrderStatusTest',
        :'old_status' => :'WebhookOrderStatusTest',
        :'new_status' => :'WebhookOrderStatusTest',
        :'order_id' => :'String',
        :'shipments' => :'Array<OrderShipmentObjectTest>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DeliverrAPI::OrderStatusWebhookEventTest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DeliverrAPI::OrderStatusWebhookEventTest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [event_time, event_id, type, update_type, old_status, new_status, order_id, shipments].hash
    end
