...
module DeliverrAPI
  class MockWebhookDetailsTest
    ...
    def self.openapi_types
      {
        :'type' => :'WebhookTypeTest',
        :'update_type' => :'WebhookMockUpdateTypeTest',
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DeliverrAPI::MockWebhookDetailsTest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DeliverrAPI::MockWebhookDetailsTest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@external_product_id.nil? && @external_product_id.to_s.length > 255
        invalid_properties.push('invalid value for "external_product_id", the character length must be smaller than or equal to 255.')
      end

      if !@external_product_id.nil? && @external_product_id.to_s.length < 1
    ...
    def valid?
      ...
      return false if !@external_product_id.nil? && @external_product_id.to_s.length > 255
      return false if !@external_product_id.nil? && @external_product_id.to_s.length < 1
      true
    end
    ...
    def hash
      [type, update_type, external_order_id, external_product_id].hash
    end
