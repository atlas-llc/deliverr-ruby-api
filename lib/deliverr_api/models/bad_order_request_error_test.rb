...
module DeliverrAPI
  class BadOrderRequestErrorTest
    attr_accessor :code

    attr_accessor :message

    # Please see <a href='https://support.deliverr.com/hc/en-us/articles/6858275452567'>Products and Order fulfillment</a> in the API guide for more information on possible error codes.
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DeliverrAPI::BadOrderRequestErrorTest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DeliverrAPI::BadOrderRequestErrorTest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
