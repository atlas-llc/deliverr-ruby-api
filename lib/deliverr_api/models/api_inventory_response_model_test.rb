...
module DeliverrAPI
  class ApiInventoryResponseModelTest
    ...
    def self.openapi_types
      {
        ...
        :'lot_tracking_details' => :'Array<LotTrackingDetailsTest>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DeliverrAPI::ApiInventoryResponseModelTest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DeliverrAPI::ApiInventoryResponseModelTest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    # Custom attribute writer method with validation
    # @param [Object] available_units Value to be assigned
    def available_units=(available_units)
      if available_units.nil?
        fail ArgumentError, 'available_units cannot be nil'
      end
    ...
    def hash
      [product_id, available_units, lot_tracking_details].hash
    end
