...
module DeliverrAPI
  class ReturnsProductIdentifiersTest
    attr_accessor :type
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DeliverrAPI::ReturnsProductIdentifiersTest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DeliverrAPI::ReturnsProductIdentifiersTest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      type_validator = EnumAttributeValidator.new('String', ["DELIVERR_SKU", "FNSKU", "SHOPIFY_PRODUCT_ID", "PRODUCT_NAME", "SHOPIFY_VARIANT_ID", "SKU", "UPC", "LOT_NUMBER", "OTHER"])
    ...
    def type=(type)
      validator = EnumAttributeValidator.new('String', ["DELIVERR_SKU", "FNSKU", "SHOPIFY_PRODUCT_ID", "PRODUCT_NAME", "SHOPIFY_VARIANT_ID", "SKU", "UPC", "LOT_NUMBER", "OTHER"])
