...
module DeliverrAPI
  class ReturnsApiProductDetailTest
    # Name of the product in the marketplace.
    attr_accessor :product_name

    # Unique Identifiers for the product it is dependent on the marketplace.
    ...
    def self.openapi_types
      {
        :'product_name' => :'String',
        :'identifiers' => :'Array<ReturnsProductIdentifiersTest>',
        :'dimensions' => :'PackageDimensionsTest'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `DeliverrAPI::ReturnsApiProductDetailTest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `DeliverrAPI::ReturnsApiProductDetailTest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [product_name, identifiers, dimensions].hash
    end
